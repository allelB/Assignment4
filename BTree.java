...
public class BTree<T extends Comparable<T>> {
    ...
    protected Deque<Node<T>> LastIn=new LinkedList<Node<T>>();
    ...
    public void insert(T value) {
        Node<T> curr_value=new Node<T>(null,maxDegree);
        if (root == null) {
            ...
            while (currentNode != null && !wasAdded) {
                ...
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                    Node<T> mid=new Node<T>(currentNode.parent,maxDegree);
                    Node<T> left=new Node<T>(mid,maxDegree);
                    Node<T> right=new Node<T>(mid,maxDegree);
                    mid.addKey(currentNode.getKey((currentNode.numOfKeys/2)));                                  //saving the key that's going up in the splitting action
                    for(int i=0;i<currentNode.numOfKeys;i++){                                                  //saving the rest of the node as a right and a left side of the split
                        if(currentNode.getKey(i).compareTo(mid.getKey(0))<0){
                            left.addKey(currentNode.getKey(i));
                        }
                        if(currentNode.getKey(i).compareTo(mid.getKey(0))>0) {
                            right.addKey(currentNode.getKey(i));
                        }
                    }
                    currentNode.SaverStack.push(right);                                                 //saving the right side split
                    currentNode.SaverStack.push(left);                                                 //saving the left side split
                    currentNode.SaverStack.push(mid);                                                 //saving the mid key
                    curr_value.SaverStack=currentNode.SaverStack;
                    split(currentNode);
            ...
            curr_value.addKey(value);

        }
        LastIn.push(curr_value);
        size++;
    }
...
//                    currentNode = currentNode.getChild(idx);
//                }
//                LastIn.push(currentNode);
//                T mid=currentNode.getKey((currentNode.numOfKeys/2));
//                currentNode.SaverStack.push(mid);
//                currentNode.SaverStack.push(value);
//            }
////            curr_value.addKey(value);
//
//        }
...
//
    ...
    protected static class Node<T extends Comparable<T>> {
        ...
        protected Deque<Node<T>> SaverStack=new LinkedList<Node<T>>();
